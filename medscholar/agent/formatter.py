"""Formatter Agent —— 引用格式化与成稿校验。

职责（需求 3.1）：

* 把正文里的数字引用标记按目标格式渲染（数字制 → ``[1]``，作者-年份制 → ``(Zhang, 2023)``）；
* 生成参考文献表（APA 7th / Vancouver / GB-T 7714 / BibTeX / RIS）；
* **校验引用完整性**：正文引用了但参考文献表没有的编号、参考文献表里有但正文从未引用的条目；
* 落盘导出多种格式。
"""
...
class FormatterAgent:
    ...
    def reference_entries(
        ...
        ``text`` 必须是**完整的参考文献文本**，而不是文内引用短标 —— 早期版本
