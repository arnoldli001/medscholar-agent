...
单独拆出来是因为"检索质量"是这个项目最常被调整的部分（放宽分级、权重、候选规模、
RRF 的 k），这些参数散在一个千行文件里时，每次调参都得先在一堆课题/会话/产物代码里
找到它们。检索日志也放在这里：它记录的就是每次检索的结果，与检索同生命周期。
...
def search_log_summary(
    *,
    ...
    用途是 PRISMA 流程里的"识别（Identification）"环节 ——
    那一步要求写出**每个数据库各检索到多少条**，而这个数字本来就在日志里，
    不该让研究者拿 Excel 手工数（数一遍半小时，改一次检索式还得重数）。
    ...
    Note:
        ``result_count`` 是**该源本次返回的条数**，同一篇文献被多个源返回会被重复计入 ——
        这正是 PRISMA 要的样子：识别阶段统计的就是"检索到的记录数"，
        去重发生在下一步（``duplicates_removed``）。把这两件事混在一起，
        会让 PRISMA 数字与流程图对不上。
    """
    sql = [
        "SELECT source,",
