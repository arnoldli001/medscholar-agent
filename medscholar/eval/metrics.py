...
关于"为什么值得单独一个模块、还要手算用例验证"：
**错误的指标比没有指标更糟** —— 它会让你把检索改动调向错误的方向，
而且看起来一切正常。所以这里的每个函数都在 `tests/test_eval_metrics.py`
里用手算结果钉死，包括边界情形（无相关文献、k 大于结果数、并列名次）。
...
把"没标注"当成"检索失败"会污染整体指标 —— 这是评测里最常见的自欺方式。
"""
