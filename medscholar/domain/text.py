...
SQLite FTS5 的 ``unicode61`` 分词器会把一整段连续汉字当成**单个 token**，
因此中文摘要（尤其 CNKI 来源）无法被关键词检索命中。

本模块的解决方式是在**索引写入**和**查询构造**两侧都插入汉字之间的空格，
使 unicode61 退化为"单字索引"，再用 FTS5 的**短语查询**（``"加 速 治 疗"``）
还原子串匹配语义。这样中英文可以共用同一张 FTS5 表，且无需任何分词依赖。
"""
...
def build_match_query(
    ...
    * 中文被切分为单字后，按 ``cjk`` 策略合并：
      ...
      ``phrase``（默认，高精度）
          相邻单字合成一个短语，等价于**连续子串**匹配。
          ``"治 疗 卒 中 后 抑 郁"`` 不会命中「治疗**脑**卒中后抑郁」。
      ``bigram``（高召回）
          相邻单字两两组成**重叠二元组**再取 AND：``"治疗" AND "疗卒" AND …``。
