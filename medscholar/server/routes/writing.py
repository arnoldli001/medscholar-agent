...
三组放在一起是因为它们共用同一条链路：**引用格式化**既服务于写作，也服务于导出；
而用户的**纠错记忆**会在下次写作时被带进提示词（不需要重新训练模型）。
"""
...
async def prisma_flow(
    ...
    系统评价/Meta 分析投稿时**必须**附 PRISMA 流程图，而研究者现在的做法是拿 Excel 手工数：
    ...
    **必须人工填的**：「题目/摘要排除」「未获取到全文」「最终纳入」——
    ...
    并在数字对不上时**提前拦住**，而不是生成一张看起来很漂亮但会被审稿人质疑的图。
    """
