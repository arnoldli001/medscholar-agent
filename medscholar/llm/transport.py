"""LLM 出网调用的**韧性**层：重试与熔断。

## 为什么单独一层

重试、熔断、限流不是"某个厂商的 API 细节"，而是**所有出网调用共有的横切关注点**。
...
一个容易写错的地方：如果在每次**尝试**失败时都记一次熔断失败，那么
"重试第 2 次成功"的调用也会留下失败记录，5 次网络抖动就能把健康的熔断器打开，
接下来 30 秒所有请求被误伤。所以这里只在**整次调用最终失败**时记一次。
（写单测时特意构造过这个场景：`attempts=3` 且第 3 次成功，熔断器必须仍是 CLOSED。）

## 流式为什么不重试

流式一开始吐字，用户屏幕上就有内容了。此时重试会让**同一段文字出现两遍**，
而且两遍内容还不一样。所以流式只在**建立连接阶段**（还没有 yield 任何字节）
...
def failure_kind(exc: BaseException) -> str:
    ...
    没有分类就无法做告警 —— "失败 37 次" 这种数字对定位问题毫无帮助，
    "429 占 30 次" 才直接指向限流。
    """
...
def http_error_kind(status: int) -> str:
    ...
    为什么不复用字符串匹配的 :func:`classify_failure`：状态码本身信息完整且无歧义，
...
async def run_with_resilience(
    ...
    *,
    ...
    对**流式**调用，调用方只把"建立连接"这一步包进来：一旦连接建立，
    读流阶段的失败不会再经过这里，因此天然不会重试（避免重复文本）。

    Args:
        send: 真正发请求的**零参可调用对象**。用可调用对象而不是 httpx 客户端，
