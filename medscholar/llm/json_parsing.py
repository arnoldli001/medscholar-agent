...
从 ``llm/client.py`` 拆出来的原因：这部分是**纯算法**（剥离围栏、截取平衡括号、
补全截断、修复尾随逗号），与"怎么发请求、怎么记账"毫无关系，
却是整个 LLM 层里最需要密集测试的一段。拆开后它可以被单独测
（``tests/test_llm_json.py``），``client.py`` 也回到 500 行以内。

## 为什么不能直接 ``json.loads``

本地小模型几乎不会严格输出纯 JSON。实测 qwen3:8b 至少四种坏法：
...
``expect="object"`` 会**只**接受顶层对象。这不是洁癖：实测残缺对象里第一个配平的
``[...]`` 恰好是 ``pico.outcomes``；不限定形状时，就会把"结局指标数组"
当成整份计划返回，``topic_zh`` / ``queries`` / ``outline`` 全部静默丢失，
表现为"规划莫名其妙只出来一个结局列表"。
"""
