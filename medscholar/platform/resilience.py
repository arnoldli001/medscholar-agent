...
本层是最底层（见 :mod:`medscholar.platform` 的依赖规则），只依赖标准库，
不导入 ``medscholar`` 的任何其他模块，因此任何层都可以安全复用它。

四个真实故障驱动了这里的四个原语：

1. 指数退避 + 全抖动：9 个外部学术 API 里总有那么一两个在改版、限流或抖动。
   固定间隔重试会把多个并发调用方的重试对齐到同一时刻，形成重试风暴，
...
3. 熔断：项目里 ``_MAX_CONCURRENT_QUERIES = 3``，一个挂掉的数据源会让每次检索
   ...
   整体吞吐直接掉 1/3；连等三轮就是分钟级卡顿。熔断让后续请求立刻失败，
...
def expo_delay(
    ...
    ``jitter=True`` 时使用全抖动（full jitter）：``rand() * capped``，
...
class CircuitBreaker:
    ...
    * ``OPEN`` —— 冷却期内一律拒绝（``allow() -> False``）；
      超过 ``reset_timeout`` 后由第一次 ``allow()`` 转入 ``HALF_OPEN``。
    * ``HALF_OPEN`` —— 只放行 ``half_open_max_calls`` 个探测请求，
      其余仍然拒绝（避免恢复瞬间把积压流量一次性灌进去，把刚缓过来的源再打死）；
      探测成功 → ``CLOSED`` 并清零；探测失败 → 重新 ``OPEN`` 并重置冷却计时。
    ...
    def _record_exception(self, exc: BaseException) -> None:
        """把异常记账为失败；``CancelledError`` 与熔断拒绝本身都不算失败。
...
class TokenBucket:
    ...
    * 约束长期平均速率——额度按 ``rate`` 每秒匀速回填，长期速率不会超过它。
    ...
    def tokens(self) -> _QuasiFloat:
        ...
        这是一个会推进内部时间基准的观察，但不会消耗令牌——供诊断面板显示。
...
class Bulkhead:
    ...
    ``peak`` 记录历史并发峰值——这是唯一可用来断言"真的并发了"的指标。
    ...
    * 异步路径（``async with``）额外挂一个 ``asyncio.Semaphore``，
      ...
      信号量在首次异步使用时惰性创建：``asyncio.Semaphore`` 会绑定当时的
