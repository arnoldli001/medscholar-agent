...
本模块提供内容：每一条提示词正文、它的版本号、中文说明、占位符声明、
以及用于 A/B 的变体。装配方向是 ``本模块 → prompts``（反向 import 会成环，
见 prompts 模块 docstring）。

迁移原则：正文一个字都不改。下面第一部分与第二部分的文本逐字迁移自
``medscholar/llm/prompts.py``：既有测试（例如"每节不少于 N 字"写进了提示词）
与线上行为都依赖这些文字，迁移只搬家、不改字。``tests/test_prompts.py`` 的
迁移完整性用例逐个断言 ``medscholar.llm.prompts`` 的常量与本模块登记文本 ``==``。

第二部分与第一部分的区别只有一个：函数型提示词（``plan_user`` 等）原本在运行时
用 f-string 拼装，这里把每次调用都会变的值留成 ``{占位符}``，其余文字原样搬过来。
调用方（兼容外壳 ``medscholar.llm.prompts``）只负责算出取值与拼接顺序，
成文的提示词一句都不留在代码里。

键命名规则：``<类别>.<用途>``，类别取自
:data:`medscholar.platform.prompts.PROMPT_KINDS`，
例如 ``plan.system`` / ``writer.section`` / ``ask.materials.empty``。
一段提示词里的分支片段（离线说明、字数区间说法、空材料兜底文案……）也各占一个 key：
它们是独立可改、独立可 A/B 的文本，塞进大模板里就又会变成"藏在代码里的字符串"。

哪些文本进注册表、哪些留在调用方，判据是"这句话会不会出现在给模型的提示词里"：
会 → 进注册表（含"（无额外要点）"这类兜底文案）；不会（变量的取值计算、
列表拼接顺序、JSON 序列化）→ 留在调用方。因此 ``_PLAN_SCHEMA`` 这类数据结构
仍在外壳里：它们是数据不是成文提示词，留在原处才能保证
``json.dumps(..., ensure_ascii=False, indent=2)`` 的输出逐字不变。

``writer.system`` 与 ``writer.section`` 额外登记了 ``variant="citation-strict"``
的 v1：正文 = 默认版正文 + 追加的引用纪律（只允许给定编号、不得编造、每个数字
都要能在材料里逐字找到）。默认变体一个字符都没动，切换只发生在进程内：
...
不用模板引擎的原因见 :mod:`medscholar.platform.prompts` 的模块 docstring：
提示词要能被逐字静态审阅，模板层会让"这版提示词到底长什么样"需要脑内渲染
才能回答。本模块的模板只有 ``{名字}`` 一种语法，替换规则是"只替换声明过的名字"。
"""
...
#: 真正需要解释"为什么改"的是后续版本，那时再写具体的中文说明。
