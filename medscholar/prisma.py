...
class PrismaFlow:
    ...
    def warnings(self) -> list[str]:
        ...
        为什么要有：PRISMA 数字必须**内部自洽**，否则审稿人一眼就能看出来。
...
def build_prisma_flow(
    *,
    ...
    这里**不做推断**：检索数、去重数、排除数都由调用方从真实数据里取。
    唯一允许推导的是 ``screened`` / ``sought_for_retrieval`` 这类**恒等式**
